...
train_file = 'dataprocess/hindi/data/train_user_hashtag_hindi_nochar.tsv'
dev_file = 'dataprocess/hindi/data/dev_user_hashtag_hindi_nochar.tsv'
test_file = 'dataprocess/hindi/data/test_user_hashtag_hindi_nochar.tsv'
...
def RNN(X_train, y_train, X_dev, y_dev, args):
    ...
    # model.add(Bidirectional(LSTM(150, recurrent_dropout=0.25,return_sequences=True)))
    model.add(Bidirectional(LSTM(150, recurrent_dropout=0.25, return_sequences=False)))
...
def change(labels):
    f = open("E:\sentimix\label/test_labels_hinglish.txt", 'r', encoding='UTF-8')
    lines = f.read().lower()
    lines = lines.lower().split('\n')[:-1]
    print(labels)
    labelcol = 1
...
if __name__ == '__main__':
    ...
    model = RNN(deepcopy(X_train), deepcopy(y_train), deepcopy(X_dev), deepcopy(y_dev),
                [MAX_FEATURES, MAXLEN, embedding_size, \
